import { addToCorpusBlacklist } from "../utils";
...
export const pricePostProcessor = (data) =>
  new Promise((resolve, reject) => {
    ...
    // Filter any stonks that don't have prices.
    const stonksWithoutPrices = data
      .filter((stock) => !stock.price)
      .map((stock) => stock.stonk);

    // If any stonks exist without prices, add them to CorpusBlacklist
    if (stonksWithoutPrices.length) addToCorpusBlacklist(stonksWithoutPrices);
